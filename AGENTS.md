...
- `dynamic-workflow`: broad multi-track work, subagents/parallel agents,
  ...
  **Adaptive loop**: For adaptive/hierarchical orchestrator work, inventory agents/tools/skills/MCPs first, assign per-packet `executionSpec`, require refined results only, record post-node replan judgments, and continue until every packet plus verification is complete. See `docs/ADAPTIVE-HIERARCHICAL-ORCHESTRATOR-OPTIMIZATIONS.md`.
