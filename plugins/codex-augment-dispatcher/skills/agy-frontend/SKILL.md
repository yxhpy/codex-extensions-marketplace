---
name: agy-frontend
description: "AGY frontend implementer. Trigger on frontend, UI, landing page, redesign, styling, CSS, animation, GSAP, ScrollTrigger, parallax, image/video assets, generated icons, interaction, responsive, browser visual verification, React/Vue/Svelte/Tailwind, 前端, 落地页, 动效, 滚动动画, 视觉检查."
---
...
ScrollTrigger, parallax, timeline choreography, image/video assets, generated
icons, icon sheets, sprite sheets, interaction, responsive, browser visual
verification, screenshot proof, React, Vue, Svelte, HTML/CSS, 前端, 落地页,
页面, 组件, 动效, 滚动动画, 视差, 视觉检查, 响应式.
...
2. Decide the visual media strategy before invoking `agy`. For image-led or cinematic work, read `references/asset-pack.md`, generate the high-quality image/video pack first, and pass local paths plus roles to `agy`. SVG and emoji are prohibited as default visual assets. If icons, badges, stickers, sprites, or game items are needed, default to generating an image_gen sheet and then use the `asset-slicer` skill plus `../../scripts/asset_slice.ts` before handing paths to AGY.
...
Use real high-quality generated media for visual-led frontends. For landing pages, brand pages, product pages, portfolios, editorial pages, games, and high-polish demos, generate the images and videos needed for hero/product/environment assets before asking `agy` to implement the UI.

Images MUST be generated with image_gen. Videos MUST be generated with Grok Video. No fallback media generation is allowed. Resource counts are unbounded. SVG and emoji are prohibited as default visual assets and must not substitute for generated media.
...
Do not use inline SVG art, SVG icon packs, emoji, CSS-only illustrations, local animation, stock clips, or placeholder shapes as default visual assets. For custom icons, badges, stickers, sprite sheets, and game items, default to image_gen sheet generation followed by `asset-slicer` slicing. Use CSS/SVG only for structural masks, diagrams, simple controls, canvas effects, or explicitly requested vector work after required generated image/video assets are present.
...
- All image assets in the manifest must come from image_gen. All video assets in the manifest must come from Grok Video. Do not invent fallback media or substitute CSS/SVG/emoji/local animation for missing generated assets.
- SVG and emoji are prohibited as default visual assets. Do not use inline SVG art, SVG icon packs, emoji glyphs, or CSS-only illustrations as visual substitutes unless the user explicitly requests them.
- For icons, badges, stickers, sprites, or game items, use image_gen to create a sheet, run `asset-slicer`, and include only files that passed `asset-slicer` plus the generated `asset-slices.json` report.
...
- Prefer high-quality generated image/video media when the task needs visual richness; generate enough assets for every major visual section before `agy` implements.
- Do not use emoji or SVG icon packs as visual polish defaults; custom icons default to generated sheets sliced through `asset-slicer`.
