...
test("main dispatch skill defines generic adapter routing without taking over Codex execution", () => {
  const text = readFileSync(path.join(PLUGIN_ROOT, "skills/dispatch/SKILL.md"), "utf8");
...
test("plugin-owned executable scripts are TypeScript only", () => {
  const scriptFiles = readdirSync(path.join(PLUGIN_ROOT, "scripts"), { withFileTypes: true })
  ...
  for (const script of scriptFiles) {
    const text = readFileSync(path.join(PLUGIN_ROOT, "scripts", script), "utf8");
    assert.doesNotMatch(text, new RegExp("playwright_cli\\." + "sh|docker_clean_test\\." + "sh"));
  }
});
...
`,
  ...
  const claudePlan = runNodeScript("scripts/task_gate.ts", ["--json", "Build merged plugin"], env);
  assert.equal(claudePlan.status, 0, claudePlan.stderr);
  assert.match(claudePlan.stdout, /Plan with fake Claude/);

  const grokCritic = runNodeScript("scripts/grok_augment.ts", ["critic", "--json", "Review merged plugin"], env);
