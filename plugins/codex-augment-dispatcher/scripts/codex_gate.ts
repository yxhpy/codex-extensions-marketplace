...
export class RouteDecision {
  ...
  constructor({
    ...
    this.requiredPlugins = requiredPlugins.map((plugin) => plugin.trim()).filter(Boolean);
    this.pluginEvidenceRequired = pluginEvidenceRequired ?? this.requiredPlugins.length > 0;
  }
}
...
export class RoutePlanner {
  ...
  classify(prompt: string): RouteDecision {
    ...
    return parseRouteOutput(this.thinker.think(buildRoutePrompt(prompt)), { sourcePrompt: prompt });
  }
}
...
export class CodexGate {
  ...
  run({
    ...
    const initialReport = ["Route decision:", formatRouteDecision(routeDecision), "Initial task plan:", plan.asNumberedText()];

    if (!execute) return new GateResult({ exitCode: 0, output: initialReport.join("\n") });
    if (maxRounds < 1) {
    ...
    for (let roundNumber = 1; roundNumber <= maxRounds; roundNumber += 1) {
      const codexPrompt = buildCodexExecutionPrompt(currentPlan, roundNumber, routeDecision);
      try {
        ...
        decision = enforcePluginEvidence(routeDecision, runResult.output, decision);
...
export function buildCodexExecutionPrompt(plan: TaskPlan, roundNumber = 1, routeDecision?: RouteDecision): string {
  const routingText = routeDecision ? `${formatRouteDecision(routeDecision)}\n\n` : "";
...
export function buildRoutePrompt(prompt: string): string {
  return (
    ...
    "- frontend: use agy-frontend for frontend build, edit, redesign, styling, layout, interaction, browser UI work, or visual verification.\n" +
...
export function parseRouteOutput(output: string, { sourcePrompt }: { sourcePrompt: string }): RouteDecision {
  if (!output.trim()) throw new PlanError("route classifier returned an empty decision");
  const parsed = tryParseJson(output);
  if (parsed === null) throw new PlanError("route classifier returned invalid JSON");
...
function routeFromJson(parsed: JsonValue, sourcePrompt: string): RouteDecision {
  ...
  if (record && asRecord(record.structured_output)) return routeFromJson(record.structured_output, sourcePrompt);
  ...
  const route = String(record.route || record.category || record.kind || "").trim();
  if (!route) throw new PlanError("route classifier JSON must contain a route");
  const requiredPlugins = stringList(record.required_plugins || record.requiredPlugins || record.plugins || []);
  const rawEvidence = record.plugin_evidence_required ?? record.pluginEvidenceRequired;
  ...
  if (typeof rawEvidence === "boolean") pluginEvidenceRequired = rawEvidence;
    ...
    pluginEvidenceRequired = ["true", "yes", "required"].includes(rawEvidence.trim().toLowerCase());
  }
  if (requiredPlugins.length > 0 && !pluginEvidenceRequired) {
    throw new PlanError("route classifier cannot require plugins without plugin evidence");
  }
  return new RouteDecision({
    route,
    reason: String(record.reason || record.rationale || `Classified prompt: ${sourcePrompt.slice(0, 80)}`).trim(),
    requiredPlugins,
    pluginEvidenceRequired,
  });
}
...
function enforcePluginEvidence(
  ...
  if (!routeDecision.pluginEvidenceRequired || hasRequiredPluginEvidence(routeDecision, codexOutput)) return decision;
  ...
  return new FollowupDecision({
    ...
    nextTasks: [
      new Task({
        ...
        acceptanceCriteria: missing.map((plugin) => `Detailed completion summary includes Plugin evidence for ${plugin}`),
      }),
    ],
  });
}

function hasRequiredPluginEvidence(routeDecision: RouteDecision, text: string): boolean {
  return missingPluginEvidence(routeDecision, text).length === 0;
}

function missingPluginEvidence(routeDecision: RouteDecision, text: string): string[] {
  const lower = text.toLowerCase();
  if (!lower.includes("plugin evidence")) return [...routeDecision.requiredPlugins];
  return routeDecision.requiredPlugins.filter((plugin) => !lower.includes(plugin.toLowerCase()));
}
...
function formatRouteDecision(routeDecision: RouteDecision): string {
  const plugins = routeDecision.requiredPlugins.length ? routeDecision.requiredPlugins.join(", ") : "none";
  return [
