...
export class GrokCli {
	...
	singleTurn(
		...
		try {
			...
			if (!output)
				throw new GrokAugmentError("grok returned an empty response");
			return output;
		} finally {
	...
	inspect(): JsonPayload {
		...
		if (!version)
			throw new GrokAugmentError("grok --version returned empty output");
		if (!models)
			throw new GrokAugmentError("grok models returned empty output");
		return {
			provider: "grok-cli",
	...
	private run(args: string[]): RunResult {
		...
		if (
			...
			completed.error?.name === "TimeoutError"
		) {
		...
		if ((completed.status ?? 0) !== 0) {
			...
			throw new GrokAugmentError(
				stderr || stdout || `grok exited with status ${completed.status}`,
			);
		}
		return completed;
	}
...
export class GrokVideoClient {
	...
	async generate(
		...
		if (!videoId)
			throw new GrokAugmentError(
				`grok video response missing id: ${JSON.stringify(job)}`,
			);
		...
		if (state !== "completed") {
			throw new GrokAugmentError(
				`grok video did not complete: status=${state || "unknown"}`,
			);
		}
		...
		if (videoBytes.length < MIN_VIDEO_BYTES) {
			throw new GrokAugmentError(
				`grok video output too small: ${videoBytes.length} bytes`,
			);
		}
	...
	private async openJson(
		...
		try {
			...
			if (!response.ok)
				throw new GrokAugmentError(
					`grok video HTTP ${response.status}: ${text.trim()}`,
				);
	...
	private async downloadVideo(
		...
		const url =
			typeof job.url === "string" && job.url
				? job.url
				: this.url(`/v1/videos/${videoId}/content`);
		const controller = new AbortController();
		...
		try {
			...
			if (!response.ok)
				throw new GrokAugmentError(
					`grok video download HTTP ${response.status}: ${body.toString("utf8").trim()}`,
				);
			return body;
		} catch (error) {
			if (error instanceof GrokAugmentError) throw error;
			throw new GrokAugmentError(
				`grok video download failed: ${String(error)}`,
			);
		} finally {
			clearTimeout(timer);
		}
...
export function buildPrompt(mode: string, brief: string): string {
	...
	if (!VALID_MODES.has(mode))
		throw new GrokAugmentError(`unknown mode: ${mode}`);
	const headers: Record<string, string> = {
		research:
			"You are Grok Research for the owner agent. Use current web/X knowledge when useful. Return concise Markdown with: Findings, source URLs, Risks, and owner-agent Actions. The owner agent owns local file edits, command execution, verification, and commits.",
		critic:
			"You are an independent Grok reviewer for the owner agent. Find missing requirements, incorrect assumptions, risk, and verification gaps. Do not edit files or ask for tool execution. The owner agent owns local file edits and tests.",
		creative:
			"You are Grok Creative for an owner-agent/AGY workflow. Produce bold but executable visual directions, interaction ideas, copy, and asset prompts. For image assets, state that the owner agent must use image_gen. For video assets, state that the owner agent must use Grok video. Do not cap asset count unless the user asks.",
		video:
			"You are Grok Video for the owner agent. Produce a video-generation brief and shot list for Grok video only. No fallback provider is allowed. Do not use image providers for video. Include prompt, duration, aspect ratio, camera movement, style, negative constraints, and expected output checks.",
		diverge:
			"You are Grok Divergence for the owner agent. Generate 3 to 7 meaningfully different candidate paths, with tradeoffs, risks, and how the owner agent can verify each locally. Do not mutate files.",
	};
	return `${headers[mode]}\n\nUser brief:\n${cleaned}`;
}
...
export function buildCli(): GrokCli {
	...
	if (!command)
		throw new GrokAugmentError("GROK_AUGMENT_GROK_BIN must not be blank");
	const timeoutSeconds = Number(
		process.env.GROK_AUGMENT_TIMEOUT || String(DEFAULT_TIMEOUT_SECONDS),
	);
...
function buildVideoClient(args: CliArgs): GrokVideoClient {
	const baseUrl =
		args.baseUrl ||
		process.env.GROK_VIDEO_BASE_URL ||
		DEFAULT_GROK_VIDEO_BASE_URL;
	const apiKeyEnv = args.apiKeyEnv || "GROK_VIDEO_API_KEY";
	const timeoutSeconds = Number(
		process.env.GROK_VIDEO_TIMEOUT || String(DEFAULT_TIMEOUT_SECONDS),
	);
...
function parseArgs(argv: string[]): CliArgs {
	...
	const consumeFlag = (item: string, index: number): number => {
		if (item === "--json") args.json = true;
			...
			args.model = argv[++index] || DEFAULT_GROK_VIDEO_MODEL;
		else if (item === "--seconds")
			args.seconds = Number(argv[++index] || DEFAULT_GROK_VIDEO_SECONDS);
		else if (item === "--size")
			args.size = argv[++index] || DEFAULT_GROK_VIDEO_SIZE;
		else if (item === "--quality")
			args.quality = argv[++index] || DEFAULT_GROK_VIDEO_QUALITY;
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
	try {
		...
		if (args.mode === "video-generate") {
			const brief =
				args.brief.join(" ").trim() || readFileSync(0, "utf8").trim();
			const outDir =
				args.outDir ||
				path.join(tmpdir(), `grok-augment-video-${process.pid}-${Date.now()}`);
		...
		const response = cli.singleTurn(prompt, {
			effort: args.effort,
			disableWebSearch: args.mode !== "research",
		});
