...
function run(command: string, args: string[], options: Omit<SpawnSyncOptionsWithStringEncoding, "encoding"> = {}) {
...
test("Codex CLI installs the local marketplace in an isolated HOME and installed scripts run", { timeout: 60_000 }, (t) => {
  if (!hasCommand("codex")) {
  ...
  const marketplace = run("codex", ["plugin", "marketplace", "add", REPO_ROOT], { env });
  assert.equal(marketplace.status, 0, marketplace.output);
  assert.ok(marketplace.output.includes(`Added marketplace \`${MARKETPLACE_NAME}\``), marketplace.output);

  const addPlugin = run("codex", ["plugin", "add", `${PLUGIN_NAME}@${MARKETPLACE_NAME}`], { env });
  assert.equal(addPlugin.status, 0, addPlugin.output);
  assert.ok(addPlugin.output.includes(`Added plugin \`${PLUGIN_NAME}\``), addPlugin.output);

  const installedRootMatch = addPlugin.output.match(/Installed plugin root:\s*(.+)/);
  ...
  assert.ok(existsSync(manifestPath), `missing installed manifest at ${manifestPath}`);
  assert.equal(readJson(manifestPath).version, VERSION);

  const list = run("codex", ["plugin", "list", "--marketplace", MARKETPLACE_NAME], { env });
  ...
  const planner = run(
    process.execPath,
    ["--experimental-strip-types", path.join(installedRoot, "scripts/task_gate.ts"), "--json", "Plan isolated Codex E2E"],
    {
      cwd: installedRoot,
      env: {
        ...env,
...
test("Pi CLI installs the local package in an isolated config and skill-relative scripts run", { timeout: 60_000 }, (t) => {
  if (!hasCommand("pi")) {
  ...
  const resolvedPackages = packages.filter(Boolean).map((entry: string) => path.resolve(piDir, entry));
  assert.ok(resolvedPackages.includes(REPO_ROOT), `Pi settings packages did not include ${REPO_ROOT}`);

  const pkg = readJson(path.join(REPO_ROOT, "package.json"));
  assert.equal(pkg.version, VERSION);
  assert.deepEqual(pkg.pi.skills, ["./plugins/codex-augment-dispatcher/skills"]);

  const skillRoot = path.join(REPO_ROOT, "plugins/codex-augment-dispatcher/skills/task-gate");
  const skillRelativeScript = path.resolve(skillRoot, "../../scripts/task_gate.ts");
  assert.ok(existsSync(skillRelativeScript), `missing skill-relative script at ${skillRelativeScript}`);

  const fakeClaude = writeFakeClaude(home);
  const planner = run(
    process.execPath,
    ["--experimental-strip-types", skillRelativeScript, "--json", "Plan isolated Pi E2E"],
    {
      cwd: skillRoot,
      env: {
        ...env,
